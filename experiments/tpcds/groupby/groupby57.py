...
def build_models(sqlExecutor):
    # 10k
    sqlExecutor.execute(
        "create table ss40g_600k_test(ss_sales_price real, ss_sold_date_sk real) from '/data/tpcds/40G/ss_600k.csv' GROUP BY ss_store_sk method uniform size 600000")
...
def query(sqlExecutor):
    sqlExecutor.execute("select count(ss_sales_price)  from ss40g_600k_gg_test where ss_sold_date_sk between 2451119  and 2451483   group by ss_store_sk")
