...
def build_models(sqlExecutor):
    # 10k
    sqlExecutor.execute(
        "create table ss40g(ss_sales_price real, ss_sold_date_sk real) from '/data/tpcds/40G/ss_600k.csv' GROUP BY ss_store_sk,ss_quantity method uniform size 6000", n_mdn_layer_node=8, b_one_hot_encoding=True, b_grid_search=False, device='cpu', b_use_gg=False, n_per_gg=60)
